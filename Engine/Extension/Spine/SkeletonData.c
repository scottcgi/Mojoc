...
static void Release(SkeletonData* skeletonData)
{
	...
	for (int i = 0; i < boneDataMap->arrayList->size; i++)
	{
	...
	for (int i = 0; i < slotDataMap->arrayList->size; i++)
	{
	...
	{
		for(int i = 0; i < eventDataMap->arrayList->size; i++)
		{
	...
	for (int i = 0; i < skinDataMap->arrayList->size; i++)
	{
		...
		for (int j = 0; j < slotAttachmentMap->arrayList->size; j++)
		{
			ArrayStrMap* attachmentDataMap = AArrayStrMapGetAt(slotAttachmentMap, j, ArrayStrMap*);

			for (int k = 0; k < attachmentDataMap->arrayList->size; k++)
			{
				free(AArrayStrMapGetAt(attachmentDataMap, k, SkeletonAttachmentData*)->subPtr);
			}
	...
	for (int j = 0; j < animationDataMap->arrayList->size; j++)
	{
...
static SkeletonAttachmentData* GetAttachmentDataBySkinData(SkeletonSkinData* skinData, const char* slotName, const char* attachmentName)
{
	ArrayStrMap* slotAttachmentMap = skinData->slotAttachmentMap;
...
static inline void ReadSlotData(JsonObject* root, SkeletonData* skeletonData)
{
	...
	for (int i = 0; i < slotArr->arrayList->size; i++)
	{
		...
		const char* name           = AJsonObject->GetString(jsonSlot, "name",       NULL);
		const char* attachmentName = AJsonObject->GetString(jsonSlot, "attachment", NULL);
		int   nameLength           = strlen(name) + 1;
...
static inline SkeletonAttachmentData* CreateAttachmentData(int length, SkeletonAttachmentDataType attachmentDataType)
{
	...
	switch (attachmentDataType)
	{
		case skeleton_attachment_region:
		{
			...
			attachmentData->subPtr = regionAttachmentData;
		...
		case skeleton_attachment_boundingbox:
		{
			...
			attachmentData->subPtr = boundingboxAttachmentData;
		...
		case skeleton_attachment_mesh:
		{
			...
			attachmentData->subPtr                         = meshAttachmentData;
		...
		case skeleton_attachment_skinned_mesh:
		{
			...
			attachmentData->subPtr                                       = skinnedMeshAttachmentData;
...
{
	ArrayStrMap* attachmentDataMap = AArrayStrMap->CreateWithCapacity(sizeof(SkeletonAttachmentData*), attachmentDataMapObject->arrayStrMap->arrayList->size);

	for (int i = 0; i < attachmentDataMapObject->arrayStrMap->arrayList->size; i++)
	{
		...
		const char* attachmentType       = AJsonObject->GetString       (attachmentDataObject,    "type", NULL);
		...
		else if (strcmp(attachmentType, "boundingbox") == 0)
		{
			...
			SkeletonBoundingboxAttachmentData* boundingboxAttachmentData = (SkeletonBoundingboxAttachmentData*) attachmentData->subPtr;
		...
		attachmentData->name = ((char*) attachmentData->subPtr) + length;
...
{
	ArrayStrMap* slotAttachmentMap = skinData->slotAttachmentMap;
	AArrayStrMap->InitWithCapacity(sizeof(ArrayStrMap*), skinSlot->arrayStrMap->arrayList->size, slotAttachmentMap);

	for (int i = 0; i < skinSlot->arrayStrMap->arrayList->size; i++)
	{
...
static inline void ReadSkinData(JsonObject* root, SkeletonData* skeletonData)
{
	...
	ArrayStrMap* skinDataMap     = skeletonData->skinDataMap;
	AArrayStrMap->InitWithCapacity(sizeof(SkeletonSkinData*), skinDataObject->arrayStrMap->arrayList->size, skinDataMap);

	for (int i = 0; i < skinDataObject->arrayStrMap->arrayList->size; i++)
	{
...
)
{
	...
	for (int i = 0; i < jsonBones->arrayStrMap->arrayList->size; i++)
	{
		...
		for (int j = 0; j < jsonBoneTimeline->arrayStrMap->arrayList->size; j++)
		{
...
)
{
	...
	for (int i = 0; i < jsonSlots->arrayStrMap->arrayList->size; i++)
	{
		...
		for (int j = 0; j < jsonSlotTimeline->arrayStrMap->arrayList->size; j++)
		{
...
)
{
	...
	for (int i = 0, frameIndex = 0; i < jsonEvents->arrayList->size; i++, frameIndex++)
	{
		...
		const char* eventName        = AJsonObject->GetString(eventObject, "name", NULL);
	...
	for (int i = 0, frameIndex = 0; i < jsonDrawOrder->arrayList->size; i++, frameIndex++)
	{
		...
		if (offsets != NULL)
		{
			...
			for (int j = 0; j < offsets->arrayList->size; j++)
			{
				...
				const char* slotName     = AJsonObject->GetString(offset,  "slot",   NULL);
...
{
	for (int i = 0; i < jsonDeform->arrayStrMap->arrayList->size; i++)
	{
		...
		for (int j = 0; j < jsonSlots->arrayStrMap->arrayList->size; j++)
		{
			...
			for (int k = 0; k < jsonMeshs->arrayStrMap->arrayList->size; k++)
			{
				JsonArray*              jsonMeshArr    = AJsonObject->GetArrayByIndex(jsonMeshs, k);
				const char*             meshName       = AJsonObject->GetKey         (jsonMeshs, k);
				...
				switch (attchmentData->type)
				{
					...
					case skeleton_attachment_mesh:
					{
						meshVertices = ((SkeletonMeshAttachmentData*) attchmentData->subPtr)->vertexArr;
					}
					break;
					...
					case skeleton_attachment_skinned_mesh:
					{
						meshVertices = ((SkeletonSkinnedMeshAttachmentData*) attchmentData->subPtr)->weightVertexArr;
					}
					break;
				}
...
static inline void ReadAnimationData(JsonObject* root, SkeletonData* skeletonData)
{
	...
	(
        ...
        animationDataObject->arrayStrMap->arrayList->size,
        animationDataMap
    );

	for (int i = 0; i < animationDataObject->arrayStrMap->arrayList->size; i++)
	{
...
static inline void ReadEventData(JsonObject* root, SkeletonData* skeletonData)
{
	...
	if (eventObject != NULL)
	{
		size = eventObject->arrayStrMap->arrayList->size;
	}
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (eventValue->arrayStrMap->arrayList->size == 0)
		{
			continue;
		}
...
static inline void InitAtlas(SkeletonData* skeletonData, const char* atlasPath)
{
	...
	for (int i = 0; i < slotDataOrderArr->length; i++)
	{
		...
		for (int j = 0; j < skinDataMap->arrayList->size; j++)
		{
			const char*       skinName          = AArrayStrMap->GetKey(skinDataMap, j);
			...
			for (int k = 0; k < attachmentMap->arrayList->size; k++)
			{
