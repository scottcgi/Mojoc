...
static inline SkeletonCurveType GetCurveType(SkeletonCurveTimeline* curveTimeline, int frameIndex)
{
	...
	int type = (int) AArrayGet(curveTimeline->curveArr, index, float);
...
{
	ALogD("frameIndex[%d] curve[%f, %f, %f, %f]", frameIndex, cx1, cy1, cx2, cy2);
...
{
	float* frames          = AArrayGetData(rotateTimeline->frameArr, float);
...
{
	SkeletonRotateTimeline* rotateTimeline = (SkeletonRotateTimeline*) skeletonTimeline->subPtr;
	...
	SkeletonBone*     bone     = AArrayGetPtr(skeleton->boneArr, rotateTimeline->boneIndex, SkeletonBone);
...
static SkeletonRotateTimeline* CreateRotate(int frameCount, int boneIndex)
{
	...
	rotateTimeline->skeletonTimeline->subPtr   = rotateTimeline;
...
{
	float* frames          = AArrayGetData(translateTimeline->frameArr, float);
...
{
	SkeletonTranslateTimeline* translateTimeline = (SkeletonTranslateTimeline*) skeletonTimeline->subPtr;
	float*                     frames            = AArrayGetData(translateTimeline->frameArr, float);
	...
	SkeletonBone*     bone        = AArrayGetPtr(skeleton->boneArr, translateTimeline->boneIndex, SkeletonBone);
	SkeletonBoneData* boneData    = bone->boneData;
	Drawable*         drawable    = bone->drawable;
...
static inline void TranslateTimelineInit(SkeletonTranslateTimeline* translateTimeline, int frameCount, int boneIndex)
{
	...
	translateTimeline->skeletonTimeline->subPtr   = translateTimeline;
...
{
	SkeletonScaleTimeline*     scaleTimeline     = (SkeletonScaleTimeline*) skeletonTimeline->subPtr;
...
{
	SkeletonScaleTimeline* scaleTimeline = (SkeletonScaleTimeline*) skeletonTimeline->subPtr;
	skeletonTimeline->subPtr = scaleTimeline->translateTimeline;
...
static SkeletonScaleTimeline* CreateScale(int frameCount, int boneIndex)
{
	...
	scaleTimeline->translateTimeline->skeletonTimeline->subPtr  = scaleTimeline;
...
{
	float* frames                      = AArrayGetData(colorTimeline->frameArr, float);
...
{
	SkeletonColorTimeline* colorTimeline = (SkeletonColorTimeline*) skeletonTimeline->subPtr;
	float*                 frames        = AArrayGetData(colorTimeline->frameArr, float);
	...
	SkeletonSlot* slot = AArrayGetPtr(skeleton->slotArr, colorTimeline->slotIndex, SkeletonSlot);
...
static SkeletonColorTimeline* CreateColor(int frameCount)
{
	...
	colorTimeline->skeletonTimeline->subPtr   = colorTimeline;
...
static void SetAttachmentFrame(SkeletonAttachmentTimeline* attachmentTimeline, int frameIndex, float time, char* attachmentName)
{
	...
	AArraySet(attachmentTimeline->frameArr,          frameIndex, time,     float);
	AArraySet(attachmentTimeline->attachmentNameArr, frameIndex, copyName, char*);
}

static void AttachmentApply(SkeletonTimeline* skeletonTimeline, Skeleton* skeleton, float time, float mixPercent)
{
	SkeletonAttachmentTimeline* attachmentTimeline = (SkeletonAttachmentTimeline*) skeletonTimeline->subPtr;
	float*                      frames             = AArrayGetData(attachmentTimeline->frameArr, float);
...
static void AttachmentRelease(SkeletonTimeline* skeletonTimeline )
{
	...
	{
		free(AArrayGet(attachmentTimeline->attachmentNameArr, i, char*));
	}
...
static SkeletonAttachmentTimeline* CreateAttachment(int frameCount)
{
	...
	attachmentTimeline->skeletonTimeline->subPtr   = attachmentTimeline;
...
{
	AArraySet(eventTimeline->frameArr, frameIndex, time,      float);
	AArraySet(eventTimeline->eventArr, frameIndex, eventData, SkeletonEventData*);
}
...
static void EventApply(SkeletonTimeline* skeletonTimeline, Skeleton* skeleton, float time, float mixPercent)
{
	...
	SkeletonEventTimeline* eventTimeline = (SkeletonEventTimeline*) skeletonTimeline->subPtr;
	float*                 frames        = AArrayGetData(eventTimeline->frameArr, float);
	...
	(
		skeleton,
		AArrayGet(eventTimeline->eventArr, frameIndex, SkeletonEventData*),
		mixPercent
	);
}
...
static void EventRelease(SkeletonTimeline* skeletonTimeline)
{
	...
	{
		free(AArrayGet(eventTimeline->eventArr, i, SkeletonEventData*));
	}
...
static SkeletonEventTimeline* CreateEvent(int frameCount)
{
	...
	eventTimeline->skeletonTimeline->subPtr  = eventTimeline;
...
{
	AArraySet(drawOrderTimeline->frameArr,     frameIndex, time,      float);
	AArraySet(drawOrderTimeline->drawOrderArr, frameIndex, drawOrder, Array(int)*);
}

static void DrawOrderRelease(SkeletonTimeline* skeletonTimeline)
{
	SkeletonDrawOrderTimeline* drawOrderTimeline = (SkeletonDrawOrderTimeline*) skeletonTimeline->subPtr;
	...
	{
		free(AArrayGet(drawOrderTimeline->drawOrderArr, i, Array(int)*));
	}
...
static void DrawOrderApply(SkeletonTimeline* skeletonTimeline, Skeleton* skeleton, float time, float mixPercent)
{
	...
	float* frames = AArrayGetData(drawOrderTimeline->frameArr, float);
	...
	Array(int)*    drawOrderArr = AArrayGet(drawOrderTimeline->drawOrderArr, frameIndex, Array(int)*);
	SkeletonSlot*  slots        = AArrayGetData(skeleton->slotArr,      SkeletonSlot);
	...
	{
		drawOrder = AArrayGetData(drawOrderArr, int);
	}
...
static SkeletonDrawOrderTimeline* CreateDrawOrder(int frameCount)
{
	...
	drawOrderTimeline->skeletonTimeline->subPtr  = drawOrderTimeline;
...
{
	AArraySet(deformTimeline->frameArr,  frameIndex, time,     float);
	AArraySet(deformTimeline->vertexArr, frameIndex, vertices, Array(float)*);
}


static void DeformRelease(SkeletonTimeline *skeletonTimeline)
{
	SkeletonDeformTimeline* deformTimeline = (SkeletonDeformTimeline*) skeletonTimeline->subPtr;
	...
	{
		free(AArrayGet(deformTimeline->vertexArr, i, Array(float)*));
	}
...
{
	SkeletonDeformTimeline* deformTimeline = (SkeletonDeformTimeline*) skeletonTimeline->subPtr;
	SkeletonSlot*           slot           = AArrayGetPtr(skeleton->slotArr, deformTimeline->slotIndex, SkeletonSlot);
	...
	float* frames = AArrayGetData(deformTimeline->frameArr, float);
	...
	switch (deformTimeline->attachmentData->type)
	{
		...
		case skeleton_attachment_boundingbox:
			ALogA(false, "deformApply wrong deformTimeline->attachmentData->attachmentDataType");
			break;
		...
		{
			SkeletonSkinnedMeshAttachmentData* skinnedMeshAttachment = (SkeletonSkinnedMeshAttachmentData*) deformTimeline->attachmentData->subPtr;
	...
	{
		// time is after last frame.
		float* lastVertex = AArrayGetData
				            (
								AArrayGet(deformTimeline->vertexArr, frameLength - 1, Array(float)*),
								float
						    );
    ...
    float* pre        = AArrayGetData
    		            (
							AArrayGet(deformTimeline->vertexArr, frameIndex - 1, Array(float)*),
							float
	    				);

    float* next       = AArrayGetData
    		            (
							AArrayGet(deformTimeline->vertexArr, frameIndex, Array(float)*),
							float
						);
...
static SkeletonDeformTimeline* CreateDeform(int frameCount)
{
	...
	deformTimeline->skeletonTimeline->subPtr   = deformTimeline;
